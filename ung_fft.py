...
def lag1_autocorr(series: np.ndarray) -> float:
    ...
    if denom == 0:
        return 0.0
    return float(np.dot(series[1:], series[:-1]) / denom)

def red_noise_spectrum(freqs: np.ndarray, variance: float, phi: float) -> np.ndarray:
    phi = np.clip(phi, -0.999, 0.999)
    ang = 2 * np.pi * freqs
    denom = 1 + phi**2 - 2 * phi * np.cos(ang)
    return variance * (1 - phi**2) / denom
...
# ----------------------------
# STATISTICAL SIGNIFICANCE OF PERIODICITY
# ----------------------------
pos_mask = freqs > 0
...
phi = lag1_autocorr(log_returns)
red_noise_bg = red_noise_spectrum(freqs_pos, variance, phi)
white_noise_bg = np.full_like(red_noise_bg, power_pos.mean())

red_noise_p = np.exp(-power_pos / red_noise_bg)
white_noise_p = np.exp(-power_pos / white_noise_bg)

red_sig = benjamini_hochberg(red_noise_p, FAP_ALPHA)
white_sig = benjamini_hochberg(white_noise_p, FAP_ALPHA)

top_pos_idx = top_idx[freqs[top_idx] > 0]
top_pos_p_red = np.exp(-power[top_pos_idx] / red_noise_spectrum(freqs[top_pos_idx], variance, phi))
top_pos_p_white = np.exp(-power[top_pos_idx] / power_pos.mean())

print(f"\n{'=' * 60}")
print("STATISTICAL PERIODICITY CHECKS (POSITIVE FREQUENCIES)")
print(f"{'=' * 60}")
print(f"Red-noise AR(1) phi: {phi:.3f}")
print(f"FDR alpha: {FAP_ALPHA:.2%}")
print(f"Significant peaks vs red noise: {int(np.sum(red_sig))} / {len(red_sig)}")
print(f"Significant peaks vs white noise: {int(np.sum(white_sig))} / {len(white_sig)}")

if top_pos_idx.size > 0:
    print("\nTop positive-frequency components (with p-values):")
    print(f"{'Rank':<6}{'Freq':<12}{'Period(d)':<12}{'P(red)':<12}{'P(white)':<12}")
    for i, idx in enumerate(top_pos_idx[:10], 1):
        freq = freqs[idx]
        period_days = (1 / abs(freq)) * 252
        p_red = top_pos_p_red[i - 1]
        p_white = top_pos_p_white[i - 1]
        print(f"{i:<6}{freq:<12.2f}{period_days:<12.1f}{p_red:<12.3g}{p_white:<12.3g}")
...
print(f"{'=' * 60}")
print(
    f"{'Rank':<6}{'Index':<8}{'Frequency':<15}"
    f"{'Period (days)':<18}{'Power':<12}"
)
print("-" * 60)
